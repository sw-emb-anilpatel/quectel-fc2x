                                     ...
                                     void* pResponse)
{
    ...
    if (isNanResponse()) {
        ...
        printf("%s: Status: %d Error Info[value %d]: %s \n", __FUNCTION__, pRsp->status, valueRcvd, pRsp->nan_error);
    } else {
        ...
        case NAN_INDICATION_SELF_TRANSMIT_FOLLOWUP:
                ...
                printf("%s: Status: %d Error Info[value %d]: %s\n", __FUNCTION__, pRspInd->reason, valueRcvd, pRspInd->nan_reason);
                break;
        case NAN_INDICATION_DISABLED:
                ...
                printf("%s: Status: %d Error Info[value %d]: %s \n", __FUNCTION__, pRspdInd->reason, valueRcvd, pRspdInd->nan_reason);
                break;
        }
    }
}
    ...
    switch (pHeader->msgId) {
        ...
        case NAN_MSG_ID_STATS_RSP:
        {
            ...
            *id = (transaction_id)pFwRsp->fwHeader.transactionId;
            ...
            printf("%s: stats_type:%d",__func__,
                  pRsp->body.stats_response.stats_type);
...
int NanCommand::handleNanResponse()
{
    ...
    printf("handleNanResponse called %p", this);
    memset(&rsp_data, 0, sizeof(rsp_data));
    //get the rsp_data
    ret = getNanResponse(&id, &rsp_data);

    printf("handleNanResponse ret:%d   status:%u   response_type:%u \n",
          ret, rsp_data.status, rsp_data.response_type);
    if (ret == 0 && (rsp_data.response_type == NAN_RESPONSE_STATS) &&
        ...
        */
        ...
        printf("%s:0x%02x master_pref 0x%02x random_factor 0x%02x hop_count %u Channel \n",
                                       ...
                                       u32 message_len)
{
    if (stats_type == NAN_STATS_ID_DE_PUBLISH) {
        ...
        if (message_len != sizeof(NanPublishStats)) {
            printf("%s: stats_type = %d invalid stats length = %u expected length = %zu\n",
                                  ...
                                  struct nlattr **tb_vendor)
{
    ...
    printf("%s: Transaction id : val %d", __FUNCTION__, id);
