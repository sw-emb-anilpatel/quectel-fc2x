...
void DiagPrint(int client, unsigned char *response, int length) 
{
	...
	if (connectionTypeOptions == SERIAL) 
		SendItDiag_Uart(client,fmtMsg,length + 3);
...
int processDiagPacket(int client, unsigned char *inputMsg, unsigned int cmdLen)
{
	...
	case version:
       ...
       case subSystemDispatch:                 
		printf(">>>> Diag packet received of type %d(subsystemDispatch) Len[%d]\n", inputMsg[0], cmdLen);
		if (IsDeviceOpened() ==0 ) {
			printf( "Device is not present! command is not processed\n" );
		...
		if (inputMsg[1] == FTMSubsystem) {
			...
			if ((((inputMsg[3] << 8) & 0xff00) | inputMsg[2]) == WLANNARTCmdCode) {
				printf( "echo NART command with WLANNARTCmdCode 0x%x to host ????\n", WLANNARTCmdCode );
	                    ...
	                    printf("CRC of packet received is: 0x%x and return 0\n", Calc_CRC_16_l(inputMsg, 17));
				...
				nrespdata = MAX_RESP_DATA_SIZE; // Set the maximum allowable size of the response
                    	...
                    	printf( "processTLVPacket - RspLen [%d]\n", nrespdata );
                    	memset(respdata, 0, sizeof(respdata));
                    	if ((myStatus = processTLVPacket(inputMsg, (cmdLen - 19 - ESCAPEcnt), respdata, &nrespdata)) > 0) {
					printf( "processTLVPacket(,,,) returns %d\n", myStatus );
					...
					printf( "TLV message processing failed :- processTLVPacket(,,,) but still return 1\n" );
					printf( "Force data up stream to see if it can keep qdart happy!!!\n" );
					printf( "Yes. It did. - just print this line to remember it is the reason why Qdart was ok\n" );
					DiagRespPrint(client, respdata, nrespdata);
					printf( "Let see the respdata if they are still matched what driver was getting\n" );
					printf( "{ " );
					...
					printf( "FTMSubsystem message not processed. Just echo back the message\n" );
					printf( "ECHO BACK inputMsg\n" );
					DiagPrint(client, inputMsg, cmdLen);
					return 1; // Return 1 since the command packet was consumed.
                	}
            } else {
                //just echo back the packet.
			printf( "inputMsg[1] ( 0x%x ) != FTMSubsystem ( 0x%x ) - Not Processed\n", inputMsg[1], FTMSubsystem );
			DiagPrint(client, inputMsg, cmdLen);
                	return 1;
            }
			//break;
