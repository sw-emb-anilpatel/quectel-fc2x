...
static int generate_hostapd_config(char *output, int output_size, struct packet_wrapper *wrapper, char *ifname) {
    ...
    if (strlen(ie_override) > 0) {
        snprintf(buffer, sizeof(buffer), "uci set wireless.@wifi-iface[%d].own_ie_override=%s", wlan_id, ie_override);
        system(buffer);
    }
...
static int configure_ap_handler(struct packet_wrapper *req, struct packet_wrapper *resp) {
    ...
    strlcpy(ifname, get_default_wireless_interface_info(), sizeof(ifname));
...
static int start_up_sta_handler(struct packet_wrapper *req, struct packet_wrapper *resp) {
    ...
    if (tlv) {
        memset(value, 0, sizeof(value));
        memcpy(value, tlv->value, tlv->len);
        freq_list_len = snprintf(freq_list, sizeof(freq_list), "freq_list=%s\n", value);
    }
    ...
    if (tlv) {
        memset(value, 0, sizeof(value));
        memcpy(value, tlv->value, tlv->len);
        preassoc_rand_addr_len = snprintf(preassoc_rand_addr, sizeof(preassoc_rand_addr), "preassoc_mac_addr=%s\n", value);
    }
    ...
    if (tlv) {
        ...
        if (freq_list_len) {
            strlcat(buffer, freq_list, sizeof(buffer));
        }

        if (preassoc_rand_addr_len) {
            strlcat(buffer, preassoc_rand_addr, sizeof(buffer));
        }

        if (ssid_len) {
            strlcat(buffer, ssid, sizeof(buffer));
        }
        len = strlen(buffer);
