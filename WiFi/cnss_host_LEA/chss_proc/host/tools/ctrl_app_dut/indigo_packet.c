...
int parse_packet(struct packet_wrapper *req, char *packet, int packet_len) {
    ...
    if (capture_packet) {
        ...
        for (i = 0; i < packet_len; i++) {
            ...
            snprintf(value, sizeof(value), "0x%02x%s", (unsigned char)(packet[i]&0x00ff), (i<packet_len-1) ? ", " : "");
            strlcat(buffer, value, sizeof(char)*buffer_len);
        }
...
int add_tlv(struct tlv_hdr *tlv, int id, int len, char *value) {
    ...
    if (tlv->value) {
        ...
        indigo_logger(LOG_LEVEL_ERROR, "malloc failed for TLV: id %d, len %d",
                      tlv->id, tlv->len);
        tlv->len = 0;
        return 1;
    }
