...
uint8_t userial_to_tcio_baud(uint8_t cfg_baud, uint32_t *baud)
{
    ...
    {
        LOG_E("userial vendor open: unsupported baud idx %i", cfg_baud);
        *baud = B115200;
        return FALSE;
    }
...
int userial_tcio_baud_to_int(uint32_t baud)
{
    ...
    case B4000000:
        ...
        break;
    default:
        LOG_E("%s: unsupported baud %d", __FUNCTION__, baud);
        break;
    }

    LOG_I("%s: Current Baudrate = %d bps", __FUNCTION__, baud_rate);
    return baud_rate;
}
...
int userial_vendor_open(char *uart_device)
{
    ...
    uint8_t stop_bits;
    int ret;
    tUSERIAL_CFG *p_cfg = &bt_default_cfg;
    vnd_userial.fd = -1;
    ...
    {
        LOG_E("userial vendor open: unsupported data bits");
        return -1;
    }
    ...
    {
        LOG_E("userial vendor open: unsupported parity bit mode");
        return -1;
    }
    ...
    {
        LOG_E("userial vendor open: unsupported stop bits");
        return -1;
    }

    //  LOG_I("userial vendor open: opening %s", vnd_userial.port_name);

    if ((vnd_userial.fd = open(uart_device, O_RDWR | O_NOCTTY)) == -1)
    {
        LOG_E("userial vendor open: unable to open %s: %s(%d)",
              vnd_userial.port_name, strerror(errno), errno);
        return -1;
    }
    //  LOG_I("%s: open(%s, ...) = %d", __func__, vnd_userial.port_name,
    //        vnd_userial.fd);

    ret = tcflush(vnd_userial.fd, TCIOFLUSH);
    // LOG_I("%s: tcflush(%d, ...) = %d", __func__, vnd_userial.fd, ret);

    ret = tcgetattr(vnd_userial.fd, &vnd_userial.termios);
    //  LOG_I("%s: tcgetattr(%d, ...) = %d", __func__, vnd_userial.fd, ret);
    ...
    // LOG_I("%s: tcsetattr(%d, ...) = %d", __func__, vnd_userial.fd, ret);

    /* set input/output baudrate */
    cfsetospeed(&vnd_userial.termios, baud);
    cfsetispeed(&vnd_userial.termios, baud);
    ret = tcsetattr(vnd_userial.fd, TCSANOW, &vnd_userial.termios);
    //  LOG_I("%s: 2nd tcsetattr(%d, ...) = %d", __func__, vnd_userial.fd, ret);

    ret = tcflush(vnd_userial.fd, TCIOFLUSH);
    //  LOG_I("%s: 2nd tcflush(%d, ...) = %d", __func__, vnd_userial.fd, ret);

#if (BT_WAKE_VIA_USERIAL_IOCTL == TRUE)
    userial_ioctl_init_bt_wake(vnd_userial.fd);
#endif

    // LOG_I("device fd = %d open", vnd_userial.fd);
...
void userial_vendor_close(void)
{
    ...
    LOG_I("device fd = %d close", vnd_userial.fd);

    if ((result = close(vnd_userial.fd)) < 0)
        LOG_E("close(fd:%d) FAILED result:%d", vnd_userial.fd, result);
...
int userial_vendor_ioctl(userial_vendor_ioctl_op_t op, int *p_data)
{
    ...
    case USERIAL_OP_FLOW_ON:
        LOG_I("## userial_vendor_ioctl: UART Flow On ");
        *p_data |= TIOCM_RTS;
        err = ioctl(vnd_userial.fd, TIOCMSET, p_data);
        break;

    case USERIAL_OP_FLOW_OFF:
        LOG_I("## userial_vendor_ioctl: UART Flow Off ");
...
int userial_set_port(char *p_conf_name, char *p_conf_value, int param)
{
    ...
    {
        LOG_E("source string is too long\n");
        return -1;
    }
    return 0;
}
...
int read_hci_event(int fd, unsigned char *buf, int size)
{
    ...
    LOG_I("%s: Wait for Command Compete Event from SOC", __FUNCTION__);
    ...
    while (1)
    {
	    ...
	    if (!is_controller_usb_intf()) {
		    if (buf[0] == 0xFD) {
			    LOG_I("%s: Got FD , responding with FC", __func__);
			    wake_byte = 0xFC;
    ...
    {
        LOG_E("%s: Unexpected event recieved rather than CC", __func__);
        return 0;
    }
    ...
    {
        LOG_E("%s: Unexpected event : protocol byte: %d", __func__, buf[1]);
        count = -1;
    }
    return count;
}
...
int read_cmd_compl_event(int fd, unsigned char *buf, int size)
{
    ...
    do
    {
        ...
        if (tot_len < 0)
        {
            LOG_E("%s: Error while reading the hci event", __func__);
            break;
        }

        if (buf[1] == EVT_CMD_COMPLETE)
        {
            LOG_D("%s: Cmd Cmpl received for opcode %0x", __func__,
                  (buf[4] | (buf[5] << 8)));
            break;
        }
        else
        {
            LOG_E("%s: Unexpected event %0x received", __func__, buf[1]);
        }
