...
#else
#ifdef CSP_BUILD
static const char *VENDOR_LIBRARY_NAME = "libbt-vendor-quec.so";
#else
static const char *VENDOR_LIBRARY_NAME = "libbt-vendor-quec.so";
#endif
#endif
...
**
*******************************************************************************/
static int userial_vendor_ioctl(int fd, userial_vendor_ioctl_op_t op, int *p_data)
{
    int err = -1;
    struct termios ti;
...
static int wait_for_data(int fd, int maxTimeOut)
{
    fd_set infids;
    ...
    )
{
    ...
    switch (edl_cmd)
    {
        ...
        case EDL_PATCH_TLV_REQ_CMD:
            ...
            fprintf(stderr, "Buffer Bit: %d:\t0x%x\n",
            segtNo, cmd[6]);
            break;
        default:
...
static void rome_extract_patch_header_info(unsigned char *buf)
{
    int index;
...
static int rome_edl_set_patch_request(int fd)
{
    int size, err;
...
static int rome_edl_patch_download_request(int fd)
{
...
static int rome_attach_rampatch(int fd)
{
    int size, err;
    unsigned char cmd[HCI_MAX_CMD_SIZE];
    unsigned char rsp[HCI_MAX_EVENT_SIZE];
...
static int rome_rampatch_reset(int fd)
{
    int size, err = 0;
    unsigned char cmd[HCI_MAX_CMD_SIZE];
...
static int rome_tlv_dnld_segment(int fd, int index, int seg_size, unsigned char wait_cc_evt)
{
   ...
   if(wait_cc_evt) {
        fprintf(stderr,"%s Read Events\n",__FUNCTION__);
        err = read_hci_event(fd, rsp, HCI_MAX_EVENT_SIZE);
          fprintf(stderr,"%s Read Events Done %d\n",__FUNCTION__,err);
        if ( err < 0) {
            fprintf(stderr, "%s: Failed to downlaod patch segment: %d!\n", __FUNCTION__, index);
            return err;
        }
...
static int rome_tlv_dnld_req(int fd, int tlv_size)
{
    ...
    fprintf(stderr, "%s: gTlv_type %d gTlv_dwndCfg %d rom 0x%x \n",
        __FUNCTION__,gTlv_type, gTlv_dwndCfg, rome_ver);
    ...
    for(i=0;i<total_segment ;i++){
        ...
        wait_cc_evt = FALSE;
        if((rome_ver = ROME_VER_3_2) && (gTlv_type == TLV_TYPE_NVM))
        {
            wait_cc_evt = TRUE;
            fprintf(stderr,"MAKE it TRUE\n");
        }
...
static int rome_1_0_nvm_tag_dnld(int fd)
{
    int i, size, err = 0;
    unsigned char rsp[HCI_MAX_EVENT_SIZE];
...
static int isSpeedValid(int speed, int *local_baud_rate, int *controller_baud_rate)
{
	switch(speed) {	
	case 9600:
...
int qca_soc_init(int fd, int speed, char *bdaddr)
{
    ...
    switch (rome_ver){
        ...
        case TUFELLO_VER_1_1:
            ...
            goto download;
        case NAPLES_VER_1_0:
            rampatch_file_path = NAPLES_RAMPATCH_TLV_1_0_0_PATH;
            nvm_file_path = NAPLES_NVM_TLV_1_0_0_PATH;
            rome_ver = ROME_VER_3_2;
            goto download;

download:
            /* check if user requested for 115.2 kbps*/
            if(speed == 115200){
                local_baud_rate = USERIAL_BAUD_115200;
                controller_baud_rate = BAUDRATE_115200;
#ifdef CSP_BUILD
		if (TUFELLO_VER_1_1 == rome_ver) {
			printf("%s: Bypass F/W downloading for TUFELLO_VER_1_1\n", __func__);
			goto out_fw;
		}
#endif
            }
