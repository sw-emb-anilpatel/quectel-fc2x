...
extern char Device[];
extern ConnectionType QDARTConnectionType;
...
void DiagPrintToStream(BYTE response[], int Length, bool check)
{
	...
	else
	{
		if ( QDARTConnectionType == SERIAL ){
			if(Diag_Write_Uart(messageToSend,messageToSend_Length))
			{
	                puts("Send failed");
	                return;
	        }
...
{	
    ...
    switch (inputDIAGMsg[0])
    {
        ...
        case 0x7B:  //diagProtLoopback:
			...
			{	
				if ( QDARTConnectionType == SERIAL ){
					if(Diag_Write_Uart(inputDIAGMsg,indexOf7E+1))
					{
			                puts("Send failed");
			                return 0;
			        }
				}
				else if( send(client_sock , inputDIAGMsg , indexOf7E + 1 , 0) < 0)
                {
                    puts("Send failed");
                    return 0;
                }
