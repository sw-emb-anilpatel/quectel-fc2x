 ...
 */
extern "C" int SerialConnection(char *dev_path, char *baudrate)
{
...
extern "C" int uart_send_hci_cmd(int fd, unsigned char *rawbuf, int rawdatalen)
{
    struct uart_t *u = NULL;
    struct termios term_attr;
    int err = 0;
...
extern "C" int uart_read_raw_data(int fd, unsigned char *buf, int size)
{
    int ret;
...
static bt_callbacks_t bt_callbacks = {
    ...
    //for Android O and USB this is not needed
...
extern "C" int USBConnection(char *dev_name)
{
    int dd,dev_id;
 ...
 *
 */
extern "C" int usb_send_hci_cmd(char *dev_name, unsigned char *rawbuf, int rawdatalen)
{
    unsigned char buf[HCI_MAX_EVENT_SIZE];
	unsigned char *ptr = buf;
	unsigned char *param_ptr = NULL;
 ...
 * return value: actual recived data length
 */
extern "C" int read_raw_data(int dd, unsigned char *buf, int size)
{
    struct hci_filter flt;
	int ret;
