...
int NanCommand::handleNanIndication()
{
    ...
    printf("handleNanIndication msg_id:%u  ", msg_id);
    switch (msg_id) {
    ...
    case NAN_INDICATION_RANGING_RESULT:
        printf("handleNanIndication received NAN_INDICATION_RANGING_RESULT msg_id:%u", msg_id);
    ...
    default:
        printf("handleNanIndication error invalid msg_id:%u", msg_id);
...
NanIndicationType NanCommand::getIndicationType()
{
    if (mNanVendorEvent == NULL) {
        printf("%s: Invalid argument mNanVendorEvent:%p",
...
int NanCommand::getNanPublishTerminated(NanPublishTerminatedInd *event)
{
    if (event == NULL || mNanVendorEvent == NULL) {
        printf("%s: Invalid input argument event:%p mNanVendorEvent:%p",
...
int NanCommand::getNanMatch(NanMatchInd *event)
{
    ...
    if (remainingLen <= 0) {
        printf("%s: No TLV's present \n",__func__);
        return WIFI_SUCCESS;
    }
    printf("%s: TLV remaining Len:%d \n",__func__, remainingLen);
    while ((remainingLen > 0) &&
           (0 != (readLen = NANTLV_ReadTlv(pInputTlv, &outputTlv)))) {
        printf("%s: Remaining Len:%d readLen:%d type:%d length:%d \n",
        ...
        case NAN_TLV_TYPE_POST_NAN_CONNECTIVITY_CAPABILITIES_RECEIVE:
            if (outputTlv.length != sizeof(u32)) {
                printf("NAN_TLV_TYPE_POST_NAN_CONNECTIVITY_CAPABILITIES_RECEIVE"
                      "Incorrect size:%d expecting %zu \n", outputTlv.length,
                      sizeof(u32));
                break;
            }
        ...
        case NAN_TLV_TYPE_POST_NAN_DISCOVERY_ATTRIBUTE_RECEIVE:
            ...
            if (ret == 0) {
                ...
                printf("NAN_TLV_TYPE_POST_NAN_DISCOVERY_ATTRIBUTE_RECEIVE"
                      "Incorrect \n");
            }
            break;
        case NAN_TLV_TYPE_FURTHER_AVAILABILITY_MAP:
            ...
            ret = getNanFurtherAvailabilityMap(outputTlv.value,
                                               ...
                                               &event->famchan[0]);
            if (ret < 0)
                printf("NAN_TLV_TYPE_FURTHER_AVAILABILITY_MAP"
                      "Incorrect \n");
            break;
        ...
        case NAN_TLV_TYPE_SDEA_CTRL_PARAMS:
            if (outputTlv.length != sizeof(u32)) {
                printf("NAN_TLV_TYPE_SDEA_CTRL_PARAMS"
                      "Incorrect size:%d expecting %zu \n", outputTlv.length,
                      sizeof(u32));
                break;
            }
        ...
        case NAN_TLV_TYPE_SDEA_SERVICE_SPECIFIC_INFO:
            ...
            break;
        default:
            printf("Unknown TLV type skipped \n");
            break;
        }
...
int NanCommand::getNanFollowup(NanFollowupInd *event)
{
    ...
    if (remainingLen <= 0) {
        printf("%s: No TLV's present",__func__);
        return WIFI_SUCCESS;
    }
    printf("%s: TLV remaining Len:%d",__func__, remainingLen);
    while ((remainingLen > 0) &&
           (0 != (readLen = NANTLV_ReadTlv(pInputTlv, &outputTlv)))) {
        printf("%s: Remaining Len:%d readLen:%d type:%d length:%d",
              __func__, remainingLen, readLen, outputTlv.type,
    ...
    while ((remainingLen > 0) &&
        ...
        case NAN_TLV_TYPE_EVENT_SELF_STATION_MAC_ADDRESS:
            if (outputTlv.length > NAN_MAC_ADDR_LEN) {
                printf("%s: Reading only first %d bytes of TLV",
        ...
        case NAN_TLV_TYPE_EVENT_JOINED_CLUSTER:
            ...
            break;
        default:
            printf("Unhandled TLV type:%d", outputTlv.type);
            break;
        }
        remainingLen -= readLen;
        pInputTlv += readLen;
    ...
    while ((remainingLen > 0) &&
        ...
        case NAN_TLV_TYPE_CLUSTER_SIZE_RSP:
            if (outputTlv.length != 2 * sizeof(u32)) {
                printf("%s: Wrong length %d in Tca Indication expecting %zu bytes",
...
int NanCommand::getNanBeaconSdfPayload(NanBeaconSdfPayloadInd *event)
{
    ...
    while ((remainingLen > 0) &&
        ...
        case NAN_TLV_TYPE_VENDOR_SPECIFIC_ATTRIBUTE_RECEIVE:
        {
            ...
            if (outputTlv.length < sizeof(u32)) {
                printf("NAN_TLV_TYPE_VENDOR_SPECIFIC_ATTRIBUTE_RECEIVE"
                                              ...
                                              NanReceivePostDiscovery *pRxDisc)
{
    ...
    if (length <= 8 || pInValue == NULL) {
        printf("%s: Invalid Arg TLV Len %d < 4",
              __func__, length);
        return -1;
    }
                                             ...
                                             NanFurtherAvailabilityChannel *pFac)
{
    ...
    if ((length == 0) || pInValue == NULL) {
        printf("%s: Invalid Arg TLV Len %d or pInValue NULL",
              __func__, length);
        return -1;
    }
    ...
    *num_chans = pInValue[0];
    if (*num_chans > NAN_MAX_FAM_CHANNELS) {
        printf("%s: Unable to accommodate numchans %d",
              __func__, *num_chans);
        return -1;
    }
    ...
    if (length < (sizeof(u8) +
        ...
        printf("%s: Invalid TLV Length", __func__);
        return -1;
    }
                                   ...
                                   NanStaParameter *pRsp)
{
    ...
    if (nanCommand == NULL) {
        printf("%s: Error NanCommand NULL", __func__);
    ...
    */
    ...
    if (ret != 0) {
        printf("%s: putNanStats Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = requestEvent();
    if (ret != 0) {
        printf("%s: requestEvent Error:%d",__func__, ret);
        goto cleanup;
    }
    ...
    if (res == ETIMEDOUT)
    {
        printf("%s: Time out happened.", __func__);
        ret = WIFI_ERROR_TIMED_OUT;
        goto cleanup;
    }
    printf("%s: NanStaparameter Master_pref:%x," \
...
int NanCommand::handleNdpIndication(u32 ndpCmdType, struct nlattr **tb_vendor)
{
    ...
    printf("handleNdpIndication msg_id:%u", ndpCmdType);
    switch (ndpCmdType) {
    ...
    case QCA_WLAN_VENDOR_ATTR_NDP_END_IND:
    {
        ...
        if (!tb_vendor[QCA_WLAN_VENDOR_ATTR_NDP_INSTANCE_ID_ARRAY]) {
            printf("%s: QCA_WLAN_VENDOR_ATTR_NDP not found", __FUNCTION__);
            return WIFI_ERROR_INVALID_ARGS;
        }

        num_ndp_ids = (u8)(nla_len(tb_vendor[QCA_WLAN_VENDOR_ATTR_NDP_INSTANCE_ID_ARRAY])/sizeof(u32));
        printf("%s: NDP Num Instance Ids : val %d", __FUNCTION__, num_ndp_ids);

        if (num_ndp_ids) {
            ndpEndInd =
                (NanDataPathEndInd *)malloc(sizeof(NanDataPathEndInd)+ (sizeof(u32) * num_ndp_ids));
            if (!ndpEndInd) {
                printf("%s: ndp_instance_id malloc Failed", __FUNCTION__);
    ...
    default:
        printf("handleNdpIndication error invalid ndpCmdType:%u", ndpCmdType);
        res = (int)WIFI_ERROR_INVALID_REQUEST_ID;
        break;
    }
    return res;
                              ...
                              NanDataPathRequestInd *event)
{
    ...
    if (event == NULL || tb_vendor == NULL) {
        printf("%s: Invalid input argument event:%p tb_vendor:%p",
    ...
    printf("%s: Service Instance id : val %d", __FUNCTION__, event->service_instance_id);
    ...
    printf("%s: Ndp Instance id: %d", __FUNCTION__, event->ndp_instance_id);
    if (tb_vendor[QCA_WLAN_VENDOR_ATTR_NDP_APP_INFO]) {
        ...
        printf("%s: NDP App Info not present", __FUNCTION__);
    }
    return WIFI_SUCCESS;
}
                              ...
                              NanDataPathConfirmInd *event)
{
    ...
    printf("%s: Service Instance id : val %d", __FUNCTION__, event->ndp_instance_id);
    ...
    printf("%s: Response code %d", __FUNCTION__, event->rsp_code);
...
int NanCommand::getNanRangeReportInd(NanRangeReportInd *event)
{
    ...
    while ((remainingLen > 0) &&
        ...
        case NAN_TLV_TYPE_NAN20_RANGING_RESULT:
            ...
            printf("getNanRangeReportInd received NAN_TLV_TYPE_NAN20_RANGING_RESULT tlv type:%u ,range_measurement_cm %u, publish_id %u  \n", outputTlv.type,event->range_measurement_cm,event->publish_id);
