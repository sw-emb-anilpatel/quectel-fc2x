...
int get_ifname(char *ifname)
{
    ...
    int n = 0;
    DIR *d;
    char iface_0[IFNAMSIZ+1];
    char iface_1[IFNAMSIZ+1];
    int qcom_iface = 0;
    ...
    while ((de = readdir(d))) {
        ...
        if (strncmp(de->d_name, "wlan", 4) == 0) {
            if (n == 0)
                strlcpy(iface_0, de->d_name, (IFNAMSIZ + 1));
            else
                strlcpy(iface_1, de->d_name, (IFNAMSIZ + 1));
                
            n++;
        }
        if (strncmp(de->d_name, "p2p", 3) == 0) {
            qcom_iface = 1;
        }
    }
    closedir(d);
    ...
    if (n == 1) {
        strlcpy(ifname, iface_0, (IFNAMSIZ + 1));
        printf("\n Using Wifi interface: %s\n", iface_0);
    }
    else {
        strlcpy(ifname, iface_1, (IFNAMSIZ + 1));
        printf("\n Using Wifi interface: %s\n", iface_1);
    }
    return 0;
}
...
int WifiCommand::response_handler(struct nl_msg *msg, void *arg) {
    ...
    int res = reply.parse();
    if (res < 0) {
        printf("Failed to parse reply message = %d", res);
        return NL_SKIP;
    } else {
...
{
    //printf("~WifiVendorCommand %p destroyed", this);
...
int WifiVendorCommand::handleEvent(WifiEvent &event)
{
    ...
    if (gnlh->cmd == NL80211_CMD_VENDOR) {
        ...
        printf("%s: Vendor event: vendor_id=0x%x subcmd=%u",
              __FUNCTION__, mVendor_id, mSubcmd);

        if (tb[NL80211_ATTR_VENDOR_DATA]) {
            mVendorData = (char *)nla_data(tb[NL80211_ATTR_VENDOR_DATA]);
            mDataLen = nla_len(tb[NL80211_ATTR_VENDOR_DATA]);
            printf("%s: Vendor data len received:%d", __FUNCTION__, mDataLen);
                                       ...
                                       mac_addr addr)
{
    if (!tb_vendor[attribute]) {
        printf("Failed to get attribute : %d", attribute);
    ...
    if (nla_len(tb_vendor[attribute]) != sizeof(mac_addr)) {
        printf("Invalid mac addr lenght\n");
        return WIFI_ERROR_INVALID_ARGS;
    }
                                 ...
                                 WifiVendorCommand **vCommand)
{
    ...
    if (vCommand == NULL) {
        printf("%s: Error vCommand NULL", __FUNCTION__);
        return WIFI_ERROR_INVALID_ARGS;
    }
