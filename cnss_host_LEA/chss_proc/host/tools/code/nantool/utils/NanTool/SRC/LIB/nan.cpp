                              ...
                              NanEnableRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanEnable Error:%d",__func__, ret);
        goto cleanup;
    }
                               ...
                               wifi_interface_handle iface)
{
    ...
    if (ret != 0) {
        printf("%s: putNanDisable Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                               ...
                               NanPublishRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanPublish Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                      ...
                                      NanPublishCancelRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanPublishCancel Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                 ...
                                 NanSubscribeRequest* msg)
{
    ...
    nanCommand = new NanCommand(wifiHandle,
                                ...
                                QCA_NL80211_VENDOR_SUBCMD_NAN);
    if (nanCommand == NULL) {
        printf("%s: Error NanCommand NULL\n", __func__);
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    if (ret != 0) {
        printf("%s: putNanSubscribe Error:%d\n",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
        printf("%s: requestEvent Error:%d \n",__func__, ret);
    }
cleanup:
    delete nanCommand;
    return (wifi_error)ret;
                                        ...
                                        NanSubscribeCancelRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanSubscribeCancel Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                         ...
                                         NanTransmitFollowupRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanTransmitFollowup Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                              ...
                              NanConfigRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanConfig Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                           ...
                           NanTCARequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanTCA Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                         ...
                                         NanBeaconSdfPayloadRequest* msg)
{
    ...
    if (ret != 0) {
        printf("%s: putNanBeaconSdfPayload Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                 ...
                                 NanStaParameter* msg)
{
    ...
    if (ret != 0) {
        printf("%s: getNanStaParameter Error:%d",__func__, ret);
        goto cleanup;
    }
                                ...
                                wifi_interface_handle iface)
{
    ...
    if (ret != 0) {
        printf("%s: putNanCapabilities Error:%d",__func__, ret);
        goto cleanup;
    }
    ret = nanCommand->requestEvent();
    if (ret != 0) {
                                   ...
                                   int debug_msg_length)
{
    ...
    nanCommand = new NanCommand(wifiHandle,
                                ...
                                QCA_NL80211_VENDOR_SUBCMD_NAN);
    if (nanCommand == NULL) {
        printf("%s: Error NanCommand NULL", __FUNCTION__);
        return WIFI_ERROR_UNKNOWN;
    }

    if (debug_msg_length <= 0) {
        printf("%s: Invalid debug message length = %d", __FUNCTION__,
                                                       debug_msg_length);
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    if (ret != 0) {
        printf("%s: putNanDebugCommand Error:%d",__FUNCTION__, ret);
        goto cleanup;
    }

    ret = nanCommand->requestEvent();
    if (ret != 0) {
        printf("%s: requestEvent Error:%d",__FUNCTION__, ret);
    }
cleanup:
    delete nanCommand;
    return (wifi_error)ret;
                                     ...
                                     NanCommand **nanCommand)
{
    ...
    *nanCommand = new NanCommand(wifiHandle,
    ...
    if (*nanCommand == NULL) {
        printf("%s: Object creation failed", __FUNCTION__);
...
wifi_error nan_data_interface_create(transaction_id id,
                                     ...
                                     char* iface_name)
{
    printf("NAN_DP_INTERFACE_CREATE");
    int ret = WIFI_SUCCESS;
    struct nlattr *nlData;
    NanCommand *nanCommand = NULL;

    if (iface_name == NULL) {
        printf("%s: Invalid Nan Data Interface Name. \n", __FUNCTION__);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    ret = nan_initialize_vendor_cmd(iface,
                                    &nanCommand);
    if (ret != WIFI_SUCCESS) {
        printf("%s: Initialization failed", __FUNCTION__);
        return (wifi_error)ret;
    }
...
wifi_error nan_data_interface_delete(transaction_id id,
                                     ...
                                     char* iface_name)
{
    printf("NAN_DP_INTERFACE_DELETE");
    int ret = WIFI_SUCCESS;
    struct nlattr *nlData;
    NanCommand *nanCommand = NULL;
...
{
    printf("NAN_DP_REQUEST_INITIATOR");
    int ret = WIFI_SUCCESS;
    ...
    if ((msg->cipher_type != NAN_CIPHER_SUITE_SHARED_KEY_NONE) &&
        ...
        printf("%s: Failed-Initiator req, missing pmk and passphrase",
               __FUNCTION__);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    if ((msg->cipher_type != NAN_CIPHER_SUITE_SHARED_KEY_NONE) &&
        ...
        printf("%s: Failed-Initiator req, missing service name for out of band request",
              __FUNCTION__);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
{
    printf("NAN_DP_INDICATION_RESPONSE");
    int ret = WIFI_SUCCESS;
    struct nlattr *nlData, *nlCfgSecurity, *nlCfgQos;
    NanCommand *nanCommand = NULL;
...
NanCommand* NanCommand::instance(wifi_handle handle)
{
    ...
    if (mNanCommandInstance == NULL) {
        mNanCommandInstance = new NanCommand(handle, 0,
                                             OUI_QCA,
                                             QCA_NL80211_VENDOR_SUBCMD_NAN);
        printf("NanCommand %p created", mNanCommandInstance);
    ...
    else
    {
        if (handle != getWifiHandle(mNanCommandInstance->mInfo)) {
            ...
            printf("Handle different, update the handle");
            mNanCommandInstance->mInfo = (hal_info *)handle;
        }
    }
    printf("NanCommand %p created already", mNanCommandInstance);
    return mNanCommandInstance;
}
...
NanCommand::~NanCommand()
{
    printf("NanCommand %p destroyed", this);
}
...
int NanCommand::setCallbackHandler(NanCallbackHandler nHandler)
{
    ...
    if (res != 0) {
        ...
        printf("%s: Unable to register Vendor Handler Vendor Id=0x%x subcmd=%u",
              __func__, mVendor_id, mSubcmd);
    }
    res = registerVendorHandler(mVendor_id, QCA_NL80211_VENDOR_SUBCMD_NDP);
    if (res != 0) {
        //error case should not happen print log
        printf("%s: Unable to register Vendor Handler Vendor Id=0x%x"
...
int NanCommand::handleEvent(WifiEvent &event)
{
    ...
    printf("%s: Subcmd=%u Vendor data len received:%d \n",
    ...
    if (mSubcmd == QCA_NL80211_VENDOR_SUBCMD_NAN){
        ...
        if (tb_vendor[QCA_WLAN_VENDOR_ATTR_NDP_SUBCMD]) {
            ndpCmdType =
                ...
                printf("%s: NDP Cmd Type : val 0x%x \n",
                ...
                case QCA_WLAN_VENDOR_ATTR_NDP_END_IND:
                    ...
                    break;
                default:
                    printf("%s: Invalid NDP subcmd response received %d \n",
                          __FUNCTION__, ndpCmdType);
                }
        }
    } else {
        //error case should not happen print log
        printf("%s: Wrong NAN subcmd received %d \n", __func__, mSubcmd);
    }
    return NL_SKIP;
}
...
u16 NANTLV_WriteTlv(pNanTlv pInTlv, u8 *pOutTlv)
{
    ...
    *pOutTlv++ = (pInTlv->type & 0xFF00) >> 8;
    ...
    printf("WRITE TLV type %u, writeLen %u \n", pInTlv->type, writeLen);

    *pOutTlv++ = pInTlv->length & 0xFF;
    *pOutTlv++ = (pInTlv->length & 0xFF00) >> 8;
    writeLen += 2;

    printf("WRITE TLV length %u, writeLen %u \n", pInTlv->length, writeLen);
    ...
    printf("WRITE TLV value, writeLen %u \n", writeLen);
    return writeLen;
}
...
u16 NANTLV_ReadTlv(u8 *pInTlv, pNanTlv pOutTlv)
{
    ...
    printf("READ TLV type %u, readLen %u\n", pOutTlv->type, readLen);

    pOutTlv->length = *pInTlv++;
    pOutTlv->length |= *pInTlv++ << 8;
    readLen += 2;

    printf("READ TLV length %u, readLen %u\n", pOutTlv->length, readLen);
