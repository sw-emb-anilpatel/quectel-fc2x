...
namespace NAN_TEST
{
        ...
        :wifiHandle_(handle)
    {
        ...
        ifaceHandle = wifi_get_iface_handle(wifiHandle_, ifname);
        //ifaceHandle = wifi_get_iface_handle(wifiHandle_, (char*)"wlan7");
        if(!ifaceHandle)
        {
...
if(strcasecmp(argv[2], NDI_CREATE) == 0)
    ...
    void NanTestSuite::nanEventMatch(NanMatchInd *event)
    {
        ...
        fprintf(stderr, "Range Measurement %d \n", event->range_info.range_measurement_cm);
        fprintf(stderr, "Range Event Type %d \n", event->range_info.ranging_event_type);
                ...
                long_options, &long_index )) != -1)
        {
            switch (opt)
            {
                case 'z':
                    ...
                    break;
                case 'i':
                   strlcpy((char*)iface_name, optarg, strlen(optarg) + 1);
