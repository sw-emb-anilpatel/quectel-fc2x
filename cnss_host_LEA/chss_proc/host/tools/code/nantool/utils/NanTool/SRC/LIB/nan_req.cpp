...
int NanCommand::putNanEnable(transaction_id id, const NanEnableRequest *pReq)
{
    ...
    printf("Message Len %zu", message_len);
    ...
    if (pReq->config_enable_ranging) {
        printf("%s: Ranging enabled \n",__func__);
        tlvs = addTlv(NAN_TLV_TYPE_SYSTEM_RANGING_ENABLED,
                      sizeof(u32),
    ...
    if (ret < 0) {
        printf("%s: put_bytes Error:%d",__func__, ret);
        cleanup();
        return ret;
    }
...
int NanCommand::putNanTCA(transaction_id id, const NanTCARequest *pReq)
{
    ...
    if (pReq->tca_type == NAN_TCA_ID_CLUSTER_SIZE) {
        ...
        printf("%s: Unrecognized tca_type:%u", __FUNCTION__, pReq->tca_type);
        cleanup();
        return WIFI_ERROR_INVALID_ARGS;
    }
                         ...
                         void *arg)
{
    ...
    tmp = nla;
    *ret = err->error;
    printf("%s: Error code:%d (%s)", __func__, *ret, strerror(-(*ret)));
    return NL_STOP;
}
...
int NanCommand::requestEvent()
{
    ...
    if (!cb) {
        printf("%s: Callback allocation failed",__func__);
        res = -1;
        goto out;
    }

    /* send message */
    printf("%s:Handle:%p Socket Value:%p", __func__, mInfo, mInfo->cmd_sock);
    ...
    u32* pChannelParamArr)
{
    ...
    if (pChannelParamArr) {
        ...
        printf("%s: Filled SocialChannelParamVal", __func__);
    ...
    u8 *pOutValue)
{
    ...
    if (pTxDisc && pOutValue) {
        ...
        printf("%s: Filled TransmitPostDiscoveryVal", __func__);
    ...
    u8 *pOutValue)
{
    ...
    if (pFam && pOutValue) {
        ...
        printf("%s: Filled FurtherAvailabilityMapVal", __func__);
                                   ...
                                   int debug_msg_length)
{
    ...
    printf("Message Len %zu\n", message_len);
    printf("%s: Debug Command Type = 0x%x \n", __func__, debug.cmd);
    printf("%s: ** Debug Command Data Start **", __func__);
    hexdump(debug.debug_cmd_data, debug_msg_length);
    printf("%s: ** Debug Command Data End **", __func__);
