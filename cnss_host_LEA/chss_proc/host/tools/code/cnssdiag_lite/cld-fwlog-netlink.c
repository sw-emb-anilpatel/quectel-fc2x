...
const char options[] =
"Options:\n\
-f, --save log to file by text format(same path with tool, folder wlan_logs)\n\
...
char napier_db_parse_path[MAX_SIZE] = { 0 };
char legacy_db_parse_path[MAX_SIZE] = { 0 };
...
static int32_t create_nl_socket()
{
    ...
    if(host_hdl.enable) {
		host_hdl.sock_fd = socket(PF_NETLINK, SOCK_RAW, host_log_socket_protocol);
...
static int initial_file_path()
{
	...
	snprintf(napier_db_parse_path, sizeof(napier_db_parse_path),
				"%s/%s", filepath, NAPIER_DB_PATH);
	snprintf(legacy_db_parse_path, sizeof(legacy_db_parse_path),
				"%s/%s", filepath, LEGACY_DB_PATH);
	return 0;
}
...
{
	printf("terminate diag (%d) received!\n", signo);
	terminated = 1;
}
...
int32_t main(int32_t argc, char *argv[])
{
	...
	static struct option long_options[] = {
		...
		{"host_log_socket_protocol",required_argument, NULL, 'p'},
		{ 0, 0, 0, 0}
	};

	while (1) {
		c = getopt_long (argc, argv, "fcx:le:p:", long_options,
				 &option_index);
		if (c == -1) break;
