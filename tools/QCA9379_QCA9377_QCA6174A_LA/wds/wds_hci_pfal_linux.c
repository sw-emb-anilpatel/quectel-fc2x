...
static int port_init_libbt(uint8 option)
{
   ...
   if (option == MODE_ANT_UART){  
		opCmd = BT_VND_OP_ANT_USERIAL_OPEN;
		ERROR("BT_VND_OP_ANT_USERIAL_OPEN ");
   }
   
   void* vendor_handle = dlopen("/system/vendor/lib64/libbt-vendor.so", RTLD_NOW); 
   if(!vendor_handle){
      ERROR("Error open libbt-vendor %s \n",dlerror());
      return -1;
   }
